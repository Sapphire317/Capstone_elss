...
describe('Starting tests for elss', function() {
  ...
  it('website title should be elss', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('elss');
    })
  });

  it('network-name should be elss@0.0.6',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('elss@0.0.6.bna');
    });
  });

  it('navbar-brand should be elss',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('elss');
    });
  });
